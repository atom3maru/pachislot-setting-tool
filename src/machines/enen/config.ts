...
const config: MachineConfig = {
  ...
  guide: {
    settingHunt: [
      '炎炎ループ初当たり確率が最重要（設定1: 1/684 → 設定6: 1/486、約1.41倍差）。必ずカウント',
      'ボーナス初当たり確率にも設定差（設定1: 1/272 → 設定6: 1/227、約1.20倍差）',
      '十字目変換発生率に設定差（設定1: 42% → 設定6: 50%）。リプレイ小V時の変換を記録',
      'ボーナス終了画面：黒野=設定4以上、ジョーカー=設定5以上、シンラ(死ノ圧)=設定6濃厚',
      'REG中キャラ紹介：黒野=設定4以上、ジョーカー=設定5以上、シンラ=設定6濃厚',
      'エンディングミニキャラ：119=設定2以上、アイリス=設定4以上、紅丸&ジョーカー=設定5以上、ショウ=設定6濃厚',
    ],
    morningCheck: [
      '天井は通常850Gだが、リセット時は650Gに短縮。さらに約1/3で88G天井に振り分け',
      'リセット後は45〜88Gが激熱ゾーン。88G天井振り分けのため必ず消化',
      'リセット台は155〜250Gも狙い目ゾーン。100G付近から期待値プラス',
      'アイキャッチパターンでモード示唆を確認。高モード示唆なら続行判断の材料',
      '前日750G以上ハマりの据え置き台は天井間近のため狙い目',
    ],
    quitTiming: [
      'ボーナス後の伝導者の罠を必ず消化。炎炎激闘後は潜伏28Gを確認してからやめる',
      '2000G消化で炎炎ループ初当たり1/700以上かつ確定演出なしなら設定1-2濃厚',
      'ボーナス終了画面が通常のみ＋獲得枚数表示なしが続くなら低設定の可能性大',
      '残り天井200G以内（通常650G〜/リセット450G〜）なら天井まで打ち切り推奨',
      'リセット後は天井650Gのため、400G付近でも続行価値あり',
    ],
  },
  ...
  playGuide: {
    ...
    reelStops: [
      ...
      { name: '十字目変換', how: '自動', stopForm: 'リプレイ小V後の次G、十字目が停止', settingDiff: '設定1: 42% → 設定6: 50%',
        reelPattern: { left: ['空', 'リプ', '空'], center: ['リプ', 'リプ', 'リプ'], right: ['空', 'リプ', '空'] } },
