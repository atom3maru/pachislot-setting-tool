...
const config: MachineConfig = {
  ...
  guide: {
    settingHunt: [
      'AT初当たり確率が最重要（設定1: 1/366 → 設定6: 1/273.1、約1.34倍差）。2000G以上で判断',
      '天撃CZ成功率に大きな設定差（設定1: 50.0% → 設定6: 60.8%）。成功/失敗を必ず記録',
      '100G毎の枠ランプ：白点灯=偶数、白点滅=奇数示唆。黄緑点滅=設定4以上、金点灯=設定6濃厚',
      'サミートロフィー：銅=設定2以上、銀=設定3以上、金=設定4以上、キリン柄=設定5以上、虹=設定6確定',
      'AT終了後天国移行率に設定差（設定1: 6.8% → 設定6: 15.0%）。256あべし以内当選を記録',
      '朝一リセット時256あべし以内当選率（設定1: 25% → 設定6: 52%）も重要な判別要素',
    ],
    morningCheck: [
      '天井は通常1536あべしだが、リセット時は1280あべしに短縮。約250あべし分お得',
      'リセット後は193〜256あべしが全モード共通の高期待度ゾーン。必ず消化',
      'リセット台は400あべし付近から期待値プラス。前日最終あべし数を確認',
      '朝一256あべし以内のAT当選率で設定推測可能（設定6は52%で約半数が当選）',
    ],
    quitTiming: [
      'AT終了後30Gのモード示唆演出を必ず確認してからやめる',
      '差枚750枚以上の場合は1G回して有利区間リセットを確認',
      '2000G消化でAT初当たり1/370以上＋ランプ白のみなら設定1-2濃厚。やめ推奨',
      '地獄モード示唆（ケンシロウ演出等）が出た場合は即ヤメ検討',
      '残り天井300あべし以内なら天井まで打ち切り推奨',
    ],
  },
